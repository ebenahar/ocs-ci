...
def create_pod(desired_status=defaults.STATUS_RUNNING, wait=True, **kwargs):
    """
    Create a pod
...
def create_secret(interface_type):
    ...
    """
    secret_data = dict()
    if interface_type == defaults.CEPHBLOCKPOOL:
        secret_data = defaults.CSI_RBD_SECRET.copy()
    elif interface_type == defaults.CEPHFILESYSTEM:
...
def create_pvc(sc_name):
    ...
    return create_resource(
        desired_status=defaults.STATUS_BOUND, **pvc_data
    )
